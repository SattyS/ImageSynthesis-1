...
int main (int argc, char const* argv[])
{
	...
	Point3f n0(0.00,0.90,-1*0.44),n1(0.74,-1*0.29,-1*0.60),n2(0.67,0.33,0.67),pc(8,5,10),ph(8,14,5.6);
	int a02=1,a12=1, a22=1, a21=0, a00=-1,s0=10,s1=4, s2=3;

	Point3f normal = (2*n0%(ph - pc)/s0*s0)*n0 + (2*n1%(ph - pc)/s1*s1)*n1 + (2*n2%(ph - pc)/s2*s2)*n2 + (a21/s2)*n2;
	normal.Normalize();
	cout<<"normal: ";print(normal);
	double v=acos(0.398), u = asin(-0.872/sin(v))/(2*3.14),u1 = asin(-0.285/cos(v))/(2*3.14);
	cout<<(asin(-0.285)/sqrt(1-0.398*0.398))/2*3.14<<endl;
	cout<<v<<endl<<u<<endl;
	cout<<u1<<endl;
	Point3f p0(44,88,-48),	p1(51,92,-43),	p2(52,94,-35), a=p0-p1,b=p2-p1,area=a^b;area.Normalize();Point3f dbc(49.00,91.33,-42.00);float dot=area%dbc;
	cout<<dot<<endl;
