...
SpotLight spotLight(Point3f(20,25,0),Point3f(0,0,1),60.0/180.0);
...
int main (int argc, char const* argv[])
{
	...
	int index=0,M=24,N=M; 
	int Sx=20,winIndex=0;
	int Sy=(Sx*Ymax)/Xmax;
	float x,y;
        ...
        n1.Normalize();
        int d=10;
        float f=25;  // FOCAL PLANE distance from 0,0,0
          int lx=3,ly=(lx*Ymax)/Xmax;
        Point3f npe=Pe,Pcenter=Pe+d*n2 , PcenterLens = Pe;
        npe.Normalize();//print(npe);
        Point3f P00=Pcenter-(Sx/2.0)*n0-(Sy/2.0)*n1,Pp;
        Point3f P00Lens=PcenterLens-(lx/2.0)*n0-(ly/2.0)*n1, PeLens;
        ...
        Sphere sphere00(Point3f(27,17,42),25, Color(1,1,0),1,0 , 0);  //frontmost
        Sphere sphere0(Point3f(10,-11,5),9, Color(1,1,0),1,0 , 0); //small front
        
        Sphere sphere1(Point3f(-10,-11,5),10, Color(1,1,0),1,0 , 0);  // right 2nd big
        
        Sphere sphere2(Point3f(1.25,0,25),8, Color(0,0,1),2,0, 0);  //center
        
        Sphere sphere3(Point3f(-35,37,42),32, Color(1,1,0),3,0, 0); // back biggest
        Sphere sphere4(Point3f(22,0,55),8, Color(1,1,1),2,0, 0);
        Sphere sphere5(Point3f(33,0,68),5, Color(1,1,1),1,0, 0);
        Sphere sphere6(Point3f(13,-6,25),2, Color(1,1,1),4,0,0);
        Sphere sphere7(Point3f(17,-6,25),2, Color(1,1,1),4,0,0);
        ...
        Plane plane5(Point3f(0,1,0), Point3f(0,-8,0), Color(1,1,1), "floor",0,0);
        ...
        allObjects.push_back(dynamic_cast<Object*>(&sphere00));
        allObjects.push_back(dynamic_cast<Object*>(&sphere0));
        ...
        //allObjects.push_back(dynamic_cast<Object*>(&sphere5));
        //allObjects.push_back(dynamic_cast<Object*>(&sphere6));
        //allObjects.push_back(dynamic_cast<Object*>(&sphere7));
        //allObjects.push_back(dynamic_cast<Object*>(&sphere4));
        ...
        cout<<"EEye and 1.950760,1.0972light are outside all the object \n";
            ...
            for (int J = 0; J < Ymax; J++)
            {
              ...
              for (int p = 0; p < M; p++)
              {
                for (int q = 0; q < N; q++)
                {
                  ...
                  /*if(p ==0 && q==0) PeLens = PeLens=P00Lens+(xLens*lx)*n0+(yLens*ly)*n1;
                  if(p ==0 && q==1) PeLens = PeLens=P00Lens+(xLens*lx)*n0-(yLens*ly)*n1;
                  if(p ==1 && q==0) PeLens = PeLens=P00Lens-(xLens*lx)*n0+(yLens*ly)*n1;
                  if(p ==1 && q==1) PeLens = PeLens=P00Lens-(xLens*lx)*n0-(yLens*ly)*n1;
                  */
                  //Pe = PeLens;
