...
SpotLight spotLight(Point3f(-5,25,55),Point3f(0,0,1),60.0/180.0);
...
Color rayTracerWithGeo(Ray myray, Point3f PL, vector<Object*> allObjects , int depth,  bool spotlightEnabled ,int softShadowFlag);
int main (int argc, char const* argv[])
{
            ...
            for (int I = 0; I < Xmax; I++)
	{
		for (int J = 0; J < Ymax; J++)
		{
			...
			finalColor = rayTracerWithGeo(myray, PL,  allObjects , 0,spotlightEnabled ,softShadowFlag);
...
Color rayTracerWithGeo(Ray myray, Point3f PL, vector<Object*> allObjects , int depth,  bool spotlightEnabled=false ,int softShadowFlag=0)
{
...
int numRaysOutside = geodesicDomeNew.size(), BigConst=1000000;
    ...
    Ray geoRay(interSectionPoint, geoRayDir);
    Ray jujuRay(rayStart , PL-rayStart);		// FOR SHADOW CALCULATIONS OF PLANE 
    flag=0;
    tmp.clear();
    flag=0;
    tmp.clear();
    ...
    //shadowColor=shadowColor*ratio;
       Color tmpColor(0,0,0);
		    tmpColor = tmpColor+ ((Plane*)allObjects[winIndex])->lambertShader(myray,PL);
		    Color tracedColor = rayTracer( geoRay, PL, allObjects , depth+1, spotlightEnabled , softShadowFlag);
		    //finalColor = finalColor+  ((Plane*)allObjects[winIndex])->getColor();
		    if(tracedColor.red ==1 && tracedColor.green ==1 && tracedColor.blue ==1)
		    {	finalColor = Color(1,1,1);
		    	flag = 100;
		    	break;
		    }
		    else
		        finalColor =  finalColor + tracedColor*0.5 + tmpColor*shadowColor*0.5;
                        ...
                        if(flag ==100)
                        	return finalColor;
                        else	finalColor = finalColor/float(numRaysOutside);
...
Point3f PLrayStDir = myray.origin- PL;PLrayStDir.Normalize();
	if(myray.direction.x >= 0.95*PLrayStDir.x && myray.direction.x <= 1.05*PLrayStDir.x && myray.direction.y >= 0.95*PLrayStDir.y && myray.direction.y <= 1.05*PLrayStDir.y && myray.direction.z >= 0.95*PLrayStDir.z && myray.direction.z <= 1.05*PLrayStDir.z )
	{
		cout<<"HURRAY!! ";
		return Color(1,1,1);
	}
